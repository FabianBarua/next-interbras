...
export function AddToCartButton({ product, variant, quantity = 1, className = "" }: { product: Product; variant?: Variant; quantity?: number; className?: string }) {
  ...
  return (
    <button 
      ...
      className={`relative px-4 py-2 w-full bg-primary text-primary-foreground text-sm font-bold rounded-md hover:bg-primary/90 transition-all overflow-hidden ${animating ? "scale-95" : ""} ${className}`}
    >
      {animating && (
      ...
      <span className="relative flex items-center justify-center gap-2">
        ...
        {animating ? "¡Agregado!" : "Agregar al carrito"}
      </span>
    </button>
  )
}
