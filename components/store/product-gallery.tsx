...
export function ProductGallery({ images, alt }: { images: ProductImage[]; alt: string }) {
  ...
  if (sorted.length === 0) {
    return (
      <div className="aspect-[4/3] max-w-md rounded-xl border bg-muted/20 flex items-center justify-center text-muted-foreground text-sm">
        Sin Imagen
      </div>
    )
  }
  ...
  return (
    <div ref={galleryRef} className="max-w-lg">
      ...
      <a
        ...
        data-caption={alt}
        className="block relative aspect-[4/3] rounded-xl border bg-muted/5 overflow-hidden cursor-zoom-in group"
      >
        <Image
