...
} from "@/lib/actions/admin/shipping-methods"
...
function EditRow({
  ...
  const handleSave = () => {
    setError(null)
    startTransition(async () => {
      const res = await updateShippingMethodAction(item.id, {
  ...
  return (
    <tr className="border-b bg-primary/5">
      ...
      <td className="px-4 py-2"><input type="number" step="0.01" value={price} onChange={e => setPrice(Number(e.target.value))} className="w-24 h-8 rounded border px-2 text-sm text-right" /></td>
...
function CreateRow({
  ...
  return (
    <tr className="border-b bg-green-500/5">
      ...
      <td className="px-4 py-2"><input type="number" step="0.01" value={price} onChange={e => setPrice(Number(e.target.value))} placeholder="0.00" className="w-24 h-8 rounded border px-2 text-sm text-right" /></td>
      <td className="px-4 py-2 text-center">—</td>
      <td className="px-4 py-2 text-center"><input type="number" value={sortOrder} onChange={e => setSortOrder(Number(e.target.value))} className="w-14 h-8 rounded border px-2 text-sm text-center" /></td>
      <td className="px-4 py-2 text-right space-x-2">
