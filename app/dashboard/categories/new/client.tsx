...
export function CategoryForm() {
  ...
  const [shortDescPt, setShortDescPt] = useState("")
  const [image, setImage] = useState("")
  const [svgIcon, setSvgIcon] = useState<string | null>(null)
  const [svgIconMeta, setSvgIconMeta] = useState<{ library: string; name: string } | null>(null)
  ...
  const handleSubmit = () => {
    ...
    startTransition(async () => {
      const res = await createCategoryAction({
        ...
        ...(shortDescEs || shortDescPt ? { shortDescription: { es: shortDescEs, pt: shortDescPt } } : {}),
        ...(image ? { image } : {}),
        svgIcon,
        svgIconMeta,
        sortOrder,
