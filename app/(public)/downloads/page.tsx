...
export default async function DownloadsPage() {
  ...
  return (
    <div className="container px-4 py-4 asd">
      <Breadcrumbs items={[{ label: dict.nav.downloads }]} />
      
      <div className="max-w-3xl mx-auto mt-8 mb-16">
        <h1 className="text-3xl font-bold tracking-tight mb-4">{dict.downloads.title}</h1>
        <p className="text-muted-foreground mb-8">
          {dict.downloads.description}
        </p>

        <div className="flex gap-4 mb-8">
          <input 
            type="text" 
            placeholder={dict.downloads.searchPlaceholder}
            className="flex-1 rounded-md border h-10 px-4 focus-visible:outline-none focus:ring-2 focus:ring-primary"
          />
          <button className="px-6 h-10 bg-primary text-primary-foreground font-medium rounded-md">{dict.downloads.search}</button>
        </div>
          ...
          {downloads.map(download => (
            <div key={download.id} className="p-4 flex items-center justify-between hover:bg-muted/50 transition-colors">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h3 className="font-semibold">{download.title[locale] || download.title.es}</h3>
              ...
              <a 
                ...
                download
                title={dict.downloads.downloadFile}
              >
