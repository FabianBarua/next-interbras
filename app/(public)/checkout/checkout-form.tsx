...
export function CheckoutForm({ paymentTypes, shippingMethods }: { paymentTypes: PaymentType[]; shippingMethods: ShippingMethod[] }) {
  const router = useRouter()
  const { cart, clear } = useCartStore()
  const { dict, locale } = useDictionary()
  const lp = useLocalePath()

  const SHIPPING_OPTIONS = shippingMethods.map(sm => ({
    id: sm.slug,
    label: sm.name[locale] || sm.name.es || sm.slug,
    desc: sm.description?.[locale] || sm.description?.es || "",
    price: sm.price,
  }))
  const PAYMENT_METHODS = paymentTypes.map(pt => ({
    id: pt.slug,
    label: pt.name[locale] || pt.name.es || pt.slug,
    icon: pt.icon,
  }))
  const [mounted, setMounted] = useState(false)
  const [step, setStep] = useState(1)
  const [shipping, setShipping] = useState(shippingMethods[0]?.slug ?? "")
  const [payment, setPayment] = useState(paymentTypes[0]?.slug ?? "")
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      clear()
      router.push(lp(`/checkout/confirmacion?orderId=${res.orderId}&email=${encodeURIComponent(form.email)}`))
    } catch {
